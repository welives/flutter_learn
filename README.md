...
1. `lib`文件夹是`flutter`主要代码目录，通常不会直接在根目录下创建`dart`文件，而是会根据功能点、模块、属性等创建子文件夹，在相应子文件夹中创建相应`dart`文件，当然子文件下依然可以再创建文件夹，将目标文件所属角色细化
2. 文件夹通常是小写单词，小驼峰法使用较少，不推荐使用下划线连接法
3. 通常`main`会直接创建在根目录下
4. `android`代码目录基本上也是这个规则

## 命名规范

1. 大驼峰法：类名
2. 小驼峰法：包名、函数名、变量名、参数名
3. 使用小写加下划线来命名库和文件名，例如：`app_bar.dart`
4. 一般情况下，类名是源文件名的大驼峰形式，例如：源文件`app_bar.dart`，则类名`AppBar`
5. 不要使用前缀字母，例如`mList`不推荐，应该使用更加详细的命名，如`bookList`
6. assets目录下的资源文件名，和源文件名的命名规则一样
7. 枚举类的元素，使用大写英文加下划线，例如：YEAR_MONTH_DAY

## 代码规范

1. `if else`使用大括号，尽可能不要省略
2. 不要使用 `new`
3. page 页面中（`StatelessWidget`、`StatefulWidget`）尽量将每一层和每一个`Widget`以单独函数形式调用，层次清晰分明（工具类封装的`Widget`可例外）
4. 封装简单常用的`Widget`，注意使用`@required`、`assert`
5. 常用的颜色值和宽高值，使用统一相同含义值来赋值
6. 实时处理报错（error）和警告（warning），Dart Analysis统计了所有报错和警告的具体信息。好处：一是让代码程序更健壮，二是在dart sdk升级后，可避免报错
7. 必要的简要的代码注释（不止是dart文件，还包括pubspec.yaml）
8. 编码过程中习惯性使用 `Reformat Code with dartfmt`
9. 拒绝弱命名，如`var a`，拒绝硬编码，如`if(a == 2)`
10. 善用`///[]` ， `[]`内可以说类名、变量名，在引用中可以添加注释说明
